...
我们采用 U-Net 架构，这是一种最初为生物医学图像分割设计的卷积神经网络。该架构在需要精确定位的情况下非常有效，因为它具有两个特殊结构：收缩部分用于捕捉上下文信息，扩展部分用于实现精确定位。通过实现多个由两组卷积、批归一化和 ReLU 激活函数组成的双重卷积模块，U-Net 架构在道路分割中的表现尤为突出，因为它能够捕捉到来自高程信息等地理空间数据中不同大小的复杂特征，并将它们精确定位到输出上。
